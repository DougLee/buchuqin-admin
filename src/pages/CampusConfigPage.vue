 ...
 * - Tab 按原权限键过滤（marketing/wheel/printers/campus），权限模型不变
 */
const isPlatform = computed(() => role.value === "hq" || role.value === "admin");
...
const TABS = computed(() => {
  const list: Array<{ key: string; label: string }> = [
    ...
    { key: "wechat-groups", label: "微信群码" },
  ];
  if (canSee("printers")) list.push({ key: "printers", label: "打印机" });
  if (canSee("marketing"))
    list.push({ key: "banners", label: "Banner" }, { key: "pay-ads", label: "支付广告位" });
  if (canSee("wheel")) list.push({ key: "wheel", label: "抽奖转盘" });
  return list;
});
...
async function load() {
  loading.value = true;
  try {
    bundle.value = await api.campusConfig(
      selectedCampus.value || undefined,
    );
  } catch (error) {
...
onMounted(async () => {
  if (isPlatform.value) {
    try {
      campusOptions.value = (await api.campuses()).filter(
        (c) => (c as { status?: string }).status !== "official",
      );
      selectedCampus.value = campusOptions.value[0]?.id ?? "";
    } catch {
      /* 校区列表失败不阻塞：留空走操作者本校区 */
    }
  }
  ...
  <div class="workspace">
    ...
    <section v-else-if="activeTab === 'profile'" class="cc-panel">
      <template v-if="bundle">
        <div class="cc-form">
          <label
            >校区全称
            <input v-model.trim="profileForm.name" :disabled="!isPlatform" /></label>
          <label
            >简称
            <input
        ...
        <p v-if="!isPlatform" class="form-hint"
          >基础档案仅总部/平台账号可修改</p
        >
        <button
          v-if="isPlatform"
    ...
    <section v-else-if="activeTab === 'delivery'" class="cc-panel">
      <template v-if="bundle">
        ...
        >
        <button class="btn primary" :disabled="saving" @click="saveDelivery">
          保存配送与营业配置</button
        >
      </template>
    </section>
        ...
        <table class="cc-table">
          ...
          <tbody>
            <tr v-for="s in slots" :key="s.id">
              ...
              >
              <td class="cc-actions">
                <button class="btn mini ghost" @click="toggleSlot(s)">
                  {{ s.available ? "停用" : "启用" }}</button
                ><button class="btn mini danger-btn" @click="removeSlot(s)">
                  删除</button
                ></td
              >
            </tr>
        ...
        <table class="cc-table">
          ...
          <tbody>
            <tr v-for="n in notices" :key="n.id">
              ...
              >
              <td class="cc-actions">
                <button class="btn mini ghost" @click="toggleNotice(n)">
                  {{ n.status === "active" ? "停用" : "启用" }}</button
                ><button class="btn mini danger-btn" @click="removeNotice(n)">
                  删除</button
                ></td
              >
            </tr>
